...
async function getWebtoons(
  ...
  if (week === 8) {
    for (let newWeek = 0; newWeek < 7; newWeek++) {
      const KAKAO_PAGE_URL = `https://api2-page.kakao.com/api/v2/store/day_of_week_top/list?category=10&subcategory=0&page=${page}&day=${newWeek}`;
...
export default async function kakaoPageCrawler() {
  ...
  result.push(...(await getOneDayWebtoons(8)));
